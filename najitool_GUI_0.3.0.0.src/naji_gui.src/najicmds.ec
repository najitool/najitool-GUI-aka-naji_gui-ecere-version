...
void naji_ascii(char *string, int i, int a, FILE *stream)
{
    ...
    if (string[i] == ',')
    {
        naji_ascii_coma(a, stream);
        return;
    }
    if (string[i] == '`')
    {
        naji_ascii_aposopen(a, stream);
        return;
    }
    if (string[i] == '\'')
    {
        naji_ascii_aposclose(a, stream);
        return;
    }
    if (string[i] == '.')
    {
        naji_ascii_period(a, stream);
        return;
    }
    if (string[i] == ':')
    {
        naji_ascii_colon(a, stream);
        return;
    }
    if (string[i] == ';')
    {
        naji_ascii_semicolon(a, stream);
        return;
    }
    if (string[i] == '<')
    {
        naji_ascii_lessthan(a, stream);
        return;
    }
    if (string[i] == '>')
    {
        naji_ascii_morethan(a, stream);
        return;
    }
    if (string[i] == '(')
    {
        naji_ascii_paranopen(a, stream);
        return;
    }
    if (string[i] == ')')
    {
        naji_ascii_paranclose(a, stream);
        return;
    }
    if (string[i] == '_')
    {
        naji_ascii_underscore(a, stream);
        return;
    }
    if (string[i] == '!')
    {
        naji_ascii_exclaimark(a, stream);
        return;
    }
    if (string[i] == '|')
    {
        naji_ascii_pipe(a, stream);
        return;
    }
    if (string[i] == '#')
    {
        naji_ascii_numsign(a, stream);
        return;
    }
    if (string[i] == '/')
    {
        naji_ascii_fslash(a, stream);
        return;
    }
    if (string[i] == '\\')
    {
        naji_ascii_bslash(a, stream);
        return;
    }
